from layers import *
from metrics import align_loss, class_loss, label_loss, link_loss
from inits import *
...
class AutoRGCN_Align(Model):
    def __init__(self, placeholders, input_dim, hidden_dim, output_dim, train_labels, dataset, REL=None,
        ...
        if FLAGS.optim == "GD":
            ...
            # self.optimizer = tensorflow_privacy.DPGradientDescentGaussianOptimizer(
            ...
            self.optimizer = tensorflow_privacy.DPAdamGaussianOptimizer(
                ...
                num_microbatches=1,
                learning_rate=FLAGS.learning_rate)
            # 0.00001 is the lowest l2 norm clip before breaking stuff
            # self.optimizer = tensorflow_privacy.DPAdamGaussianOptimizer(
            #     l2_norm_clip=0.00001,
    ...
    def _loss(self):
        if self.task == "align":
            ...
            self.loss += link_loss(self.outputs[0], self.train_labels[0], self.train_labels[1], self.train_labels[2], self.train_labels[3], self.names_neg[0])


    def build(self):
        """ Wrapper for _build() """
        with tf.variable_scope(self.name):
            if self.task == "align" or self.task == "link":
                self._build_align()
